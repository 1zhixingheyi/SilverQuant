...
提供完整的掘金平台回测策略框架：
- HMA均线系统：基于多周期HMA的趋势跟踪策略
- ATR风险管理：动态止损止盈机制
- 持仓管理：基于时间和收益的持仓轮动
- 技术指标集成：SMA、WMA、ATR等多指标分析
- 完整回测框架：从初始化到执行的完整流程

回测策略架构：
- 参数配置：Parameter类统一管理策略参数
- 数据准备：盘前历史数据和技术指标计算
- 买入逻辑：基于HMA均线排列的选股策略
- 卖出逻辑：多层次的止损止盈机制
- 持仓管理：持仓天数和资金分配策略
...
核心功能特性：
- 多周期HMA：20日、40日、60日HMA均线系统
- ATR动态止损：基于平均真实波幅的止损机制
- 换仓策略：持仓天数和收益率动态调整
- 风险控制：多重止损止盈保护机制
- 回测报告：完整的回测性能分析

技术指标体系：
- HMA（Hull移动平均）：减少滞后的趋势指标
- SMA（简单移动平均）：基础的趋势确认指标
- ATR（平均真实波幅）：动态风险评估指标
- WMA（加权移动平均）：HMA计算的中间指标
...
与其他模块的关系：
- mytt/MyTT_advance.py: 使用技术分析指标库
- tools/utils_cache.py: 获取股票代码池数据
- tools/utils_dfcf.py: 使用东方财富数据接口
- 掘金平台：提供回测环境和数据支持
"""
